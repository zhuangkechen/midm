...
#                yield (LabeledPoint(time_estimated, features_list))
#            else :
#                tmp_str = ""
#                yield (LabeledPoint(tmp_str, features_list))
def resplit_timeEstimated_only(words, compare_time, formula_a, formula_b) :
    tmp_id = words[0]
    tmp_time = words[1][1]
    tmp_features = words[1][0]
    if tmp_time !=None :
...
def resplit_timeEstimated(words, compare_time, formula_a, formula_b) :
    ...
    if tmp_time !=None :
        ...
        if tmp_features !=None and t1 > t2:
            ...
            for i in range(1, len(tmp_list)) :
                features_list.append(float(tmp_list[i].split(":")[1]))
            tmp_features = "%f" % (features_list[0])
            for i in range(1, len(features_list)-1) :
                tmp_tmp = "->%f" % (features_list[i])
                tmp_features = tmp_features + tmp_tmp
            yield (tmp_features, tmp_str)

def resplit_predicted_timedelay(words) :
    dif = words[1][1]
    tmp_time = words[1][0]
    if dif != None and tmp_time != None :
...
def check_predicted_timedelay(words):
    tmp_time = words[0]
    dif = words[1]
    if tmp_time != "" and int(dif) == 1 :
        predicted_time = tmp_time.split("->")[1]
        t1 = datetime.strptime(predicted_time, "%Y-%m-%d %H:%M:%S")
        the_day = (t1.month-4)*30 + t1.day
        yield (the_day, 1)

def resplit_only_feature(words):
    dif = words.label
    features = words.features
    tmp_str = ""
    if features != None :
        tmp_str = "%f" % (features[0])
        for i in range(1, len(features)-1):
            tmp_features = "->%f" % (features[i])
...
if __name__ == "__main__":
    ...
    #                                       formula_a, formula_b))
    rdd_time_estimated = rdd_sub_features\
            .flatMap(lambda words: resplit_timeEstimated_only(words, binladen_time,
                                           formula_a, formula_b))
    ...
    #        .zip(predictions)
    test_predicted = predictions.coalesce(1)
    ...
    ##
    ###
    #rdd_time_predicted = labelsAndPredictions\
    #        .flatMap(lambda words: check_predicted_timedelay(words))
    ## Here start the time delay zip
    #rdd_time_predicted = rdd_time_estimated.leftOuterJoin(featuresAndPredictions)\
