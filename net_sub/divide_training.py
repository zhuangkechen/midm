...
def resplit_libsvm(line) :
    ...
    if month == "09" or int(days) <= 21 :
        tmp_str = "%s\t1:%s\t2:%s\t3:%s\t4:%s\t5:%s\t6:%s\t7:%s\t8:%s" % \
                (dif, sP, sT, sI, sF, cT, cU, tP, tD)
        yield (tmp_str)
    #if month == "09" or int(days) <= 21 :
    #    tmp_str = "%s\t1:%s\t2:%s\t3:%s\t4:%s\t5:%s\t6:%s\t7:%s" % \
    #            (dif, sP, sT, sI, sF, cT, cU, tP)
    #    yield (tmp_str)
...
if __name__ == "__main__":
    ...
    output_path = "hdfs://node06:9000/user/function/mb_analysis/new_network_analysis/training_libsvm_tmp"
